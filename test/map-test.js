...
tape("map(selection) produces the expected result for map.json", function (
  test
) {
...
tape(
  "map(selection) produces the expected result for london-tube.json",
  function (test) {
    // Uncomment to replace the expected HTML content:
...
tape("map(selection) produces the expected result for pubs.json", function (
  test
) {
  // Uncomment to replace the expected HTML content:
...
tape("map(selection) produces the expected result for minimal.json", function (
  test
) {
  // Uncomment to replace the expected HTML content:
...
function saveBaselineHtml(jsonInPath, htmlOutPath) {
  writeFile(
    htmlOutPath,
    beautify_html(actualHtml(readFile(jsonInPath)), { indent_size: 2 })
  );
}
